...
ImporterApp.prototype.Init = function ()
{
	...
	importerButtons.AddButton ('images/right.png', 'Create VR effect', function () { myThis.GetEffect (); });
	...
	//this.ProcessFiles('model/eQuotenew.obj', true);
	...
	window.addEventListener ('drop', this.Drop.bind (this), false);
        ...
        canvas.addEventListener('dblclick', this.mousedblclick.bind(this), false);
...
ImporterApp.prototype.mousedblclick = function (e) {
  var objects = this.viewer.viewer.GetObjectsUnderMouse();
    if (objects.length > 0) 
        alert(objects[0].object.name + ' : at(' + objects[0].point.x + ',' + objects[0].point.y + ',' + objects[0].point.z + ')')
}
		...
		buttons : [
			{
				...
				callback: function (dialog) {
				    ...
				    dialog.Close();
				    ImporterApp.prototype.LoadFiles();
				}
			}
		]
	});
};
ImporterApp.prototype.LoadFiles = function()
{

				   // ImporterApp.prototype.ProcessFiles(c, false);
}
...
ImporterApp.prototype.GetEffect = function ()
{
    var test = this.viewer.viewer.renderer;
   // test.setSize(window.innerWidth/1.5, window.innerHeight/1.5);
    element = test.domElement;
...
ImporterApp.prototype.OpenFile = function ()
{
    ...
    var objFile;
    $.ajax({
        url: 'model/eQuotenew.txt',
        success: function (data) {
            objFile = data;
            //jsonData = JSM.ConvertObjToJsonData(objFile);
            //myThis.viewer.SetJsonData(jsonData);
            //var menu = document.getElementById('menu');
            //var progressBar = new ImporterProgressBar(menu);
            //myThis.JsonLoaded(progressBar);    
            $.ajax({
                url: 'model/eQuotemtl.txt',
                success: function (data) {
                    jsonData = JSM.ConvertObjToJsonData(objFile + data);
                    myThis.viewer.SetJsonData(jsonData);
                    var menu = document.getElementById('menu');
                    var progressBar = new ImporterProgressBar(menu);
                    myThis.JsonLoaded(progressBar);
